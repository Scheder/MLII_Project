...
public class Codebook implements Serializable {
	...
	public Codebook(int codebookDimension, int codebookSize){
		...
		basisVectors = 
				new Array2DRowRealMatrix(codebookDimension, codebookSize);
		...
		for(int vectorIndex = 0; vectorIndex < codebookSize; vectorIndex++){
			...
			for(int elementIndex = 0; elementIndex < codebookDimension; 
					elementIndex++){
	 ...
	 * 							"numberPartitions": Number of partitions.
	...
	public void learnUnlabeledData(FrameSet unlabeledData, 
		...
		while(!converged){
			...
			for(FrameSet batch : batches){
				double percentage = 
						Math.floor((double) batchesDone/nbBatches*100);
				if(percentage != lastPercentage){
					...
					if(percentage == 1.0){
						...
						System.out.println("Expected time to finish "
								+ "this iteration: " 
						+ ((endTime - startTime)/10000000) + "seconds");
					}
				}
	 ...
	 * complete linkage and maximum cross-variation as the distance metric. We
	 * use a cutoff so that (ceil) S/10 values remain, with S the original
	...
	public Codebook getMostInformativeSubset(){
        ...
        for(int i = 0; i < numClusters; i++){
        	...
        	int itemsToSelect = 
        			(int) Math.min(Math.ceil((double) currList.size()*0.9), 
        													currList.size());
	...
	private double getAverageRegularizedReconstructionError(
		...
		for(int columnIndex = 0; 
			...
			accumulator += Math.
					pow(difference.getColumnVector(columnIndex).getNorm(),2);
	...
	private Array2DRowRealMatrix l1RegularizedLassoSolve(FrameSet batch){
		...
		Array2DRowRealMatrix activationMatrix = new Array2DRowRealMatrix(
